...
class Projector:
    ...
    def set_input(self, hdmi_num: int):
        self._send_command(action=ACTIONS["SET"], command=COMMANDS["INPUT"],
                           data=INPUTS["HDMI1"] if hdmi_num == 1 else INPUTS["DISPLAYPORT12"])
        return True

    def set_position(self, pos_num: int):
        self._send_command(action=ACTIONS["SET"], command=COMMANDS["PICTURE_POSITION"],
                           data=POSITIONS["CUSTOM1"] if pos_num == 1 else POSITIONS["CUSTOM2"])
        return True
    ...
    def get_input(self):
        ...
        match data:
            case 2:
                return "HDMI1"
            case 6:
                return "DISPLAYPORT12"

    def get_position(self):
        data = self._send_command(action=ACTIONS["GET"], command=COMMANDS["PICTURE_POSITION"])
        match data:
            case 2:
                return "CUSTOM1"
            case 3:
